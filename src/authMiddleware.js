...
function verifyToken(req, res, next) {
  ...
  // If JWT_MANDATORY is not '1', allow a development bypass when no Authorization header
  const mandatory = process.env.JWT_MANDATORY === '1';
  if (!auth.startsWith('Bearer ')) {
    if (!mandatory) {
      // Inject a lightweight dev user for local development flows
      req.user = { sub: 'dev', role: 'dev' };
      return next();
    }
